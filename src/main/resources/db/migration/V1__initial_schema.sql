...
create table za.users (id serial constraint users_pk primary key,
                       email citext not null constraint users_pk2 unique);


create table za.toppings (id serial constraint toppings_pk primary key,
                          name citext not null constraint toppings_pk2 unique);


create table za.user_topping (user_id int not null constraint user_topping_users_id_fk references za.users,
...
create or replace function za.add_user_toppings(local_user_email text, local_user_toppings_str text) returns table (user_id integer, user_email citext,
                                                                                                                    topping_id integer, topping_name citext) as $$
declare
